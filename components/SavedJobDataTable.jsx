...
export default function SavedJobDataTable() {
    ...
    const columns = [
        ...
        {
            name: 'Action',
            cell: row => <button onClick={() => handleDelete(row?._id)} className='md:px-2 md:py-2 px-1 py-1 text-xl text-red-600 hover:text-white my-2 hover:bg-red-700 border border-red-600   rounded transition-all duration-700  '><AiFillDelete/></button>
        },
        {
            name: '',
            cell: row => <button onClick={() => router.push(`/frontend/jobDetails/${row?.job?._id}`)} className='md:px-2 md:py-2 px-1 py-1 text-xs text-black hover:text-white my-2 hover:bg-red-700 border border-indigo-600   rounded transition-all duration-700  '>view Detail</button>,
        },
    ];
