...
export default function ApplicationsDataTable({ application }) {
    ...
    const columns = [
        ...
        {
            name: 'CV',
            selector: row => <button onClick={() => handleDownloadCV(row?.cv)} className=' w-20 py-2 text-xs text-black hover:text-white my-2 hover:bg-red-700 border border-indigo-600 rounded transition-all duration-700'>Download CV</button>
        },
        {
            name: 'Status',
            selector: row => <p className={`uppercase font-semibold ${row?.status === "approved" ? "text-green-500" : ""}  ${row?.status === "rejected" ? "text-red-600" : ""}`}>{row?.status}</p>,
        },
        {
            ...
            cell: row => (
                <div className='flex items-center justify-start w-72 h-20'>
                    <button onClick={() => router.push(`/frontend/applicationDetail/${row?._id}`)} className=' w-20 py-2 mx-2 text-xs text-black hover:text-white my-2 hover:bg-red-700 border border-indigo-600 rounded transition-all duration-700'>Details</button>
                    ...
                    <button onClick={() => handleRejectStatus(row?._id)} className=' w-20 py-2 mx-2 text-xs text-red-600 hover:text-white my-2 hover:bg-red-700 border border-red-600 rounded transition-all duration-700'>Reject</button>
                </div>
            )
        },
